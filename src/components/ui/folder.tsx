...
const FolderContext = createContext<{ isOpen: boolean; size: "xxs" | "xs" | "sm" | "md" }>({ isOpen: false, size: "sm" });

export const FolderContent: React.FC<FolderContentProps> = ({ className, children }) => {
    ...
    return (
        <FolderContext.Provider value={{ isOpen, size }}>
            <div className={cn("w-full max-w-2xl flex items-center justify-center", className)}>
                <div onClick={handleClick} className={cn("group relative cursor-pointer transition-all duration-200 ease-in", !isOpen && "hover:-translate-y-1")}>
                    <div className={cn("relative", sizeClasses[size])}>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 225 171" className="absolute top-0 left-0 w-full h-full" fill={color}>
                        ...
                        <div
                            className={cn(
                                ...
                                sizeClasses[size],
                                isOpen ? "skew-x-12 scale-y-[0.6]" : "group-hover:skew-x-12 group-hover:scale-y-[0.6]"
                            )}
