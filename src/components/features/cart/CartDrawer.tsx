...
export function CartDrawer() {
    ...
    return (
        <Sheet open={isOpen} onOpenChange={toggleDrawer}>
            ...
            <SheetContent className="flex w-full flex-col sm:max-w-md border-l border-white/5 shadow-2xl bg-zinc-950/95 backdrop-blur-2xl p-0 overflow-hidden">
                <Spotlight className="absolute inset-0 pointer-events-none z-0" active />

                <div className="relative z-10 flex flex-col h-full">
                    <SheetHeader className="px-6 pt-8 pb-4 space-y-6">
                        <div className="flex items-center justify-between">
                            <div className="space-y-1">
                                <SheetTitle className="text-3xl font-display font-black text-zinc-100 uppercase tracking-tighter">Your Curation</SheetTitle>
                                <p className="text-[10px] text-zinc-500 uppercase tracking-[0.2em] font-bold">Selected Spirits & Essentials</p>
                            </div>
                    ...
                    {items.length === 0 ? (
                        <div className="flex flex-1 flex-col items-center justify-center gap-8 p-6">
                            <div className="relative">
                                ...
                                <div className="relative h-32 w-32 rounded-full bg-zinc-900 border border-white/5 flex items-center justify-center shadow-2xl">
                                    <ShoppingBag className="h-12 w-12 text-zinc-700" />
                                </div>
                            </div>
                            <div className="text-center space-y-2">
                                <h3 className="text-xl font-display font-bold text-zinc-100 uppercase tracking-tight">
                            ...
                            <Button variant="outline" className="h-12 px-8 border-white/10 hover:border-amber-500/50 hover:bg-amber-500/5 text-zinc-100 rounded-full" asChild onClick={() => toggleDrawer()}>
                        ...
                        <>
                            <ScrollArea className="flex-1 px-6">
                                ...
                                <div className="mt-8 space-y-4 pb-8">
                                    <h5 className="text-[10px] text-zinc-500 font-black uppercase tracking-[0.2em] px-1">Complete your bar</h5>
                            ...
                            <div className="pt-6 px-6 pb-8 border-t border-white/5 bg-zinc-950/50">
                                {/* Premium Gifting */}
                                <div className={cn(
                                    "rounded-2xl p-4 border transition-all duration-500 mb-6",
                                    isGifting ? "bg-amber-500/10 border-amber-500/30 shadow-[0_0_20px_rgba(245,158,11,0.1)]" : "bg-zinc-900/50 border-white/5"
                                )}>
                                    <div className="flex items-start justify-between">
                                        <div className="flex items-start gap-3">
                                            <div className={cn(
                                                "p-2 rounded-xl transition-colors",
                                                isGifting ? "bg-amber-500 text-black" : "bg-zinc-800 text-zinc-400"
                                            )}>
                                                <Gift className="h-4 w-4" />
                                            </div>
                                            <div>
                                                <h5 className="text-xs font-bold text-zinc-100">Boutique Gifting</h5>
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                isGifting ? "bg-amber-500" : "bg-zinc-800"
                                            )}
                                        >
                                            <span className={cn(
                                    ...
                                    {isGifting && (
                                        ...
                                        >
                                            <textarea
                                                ...
                                                className="w-full min-h-[80px] p-3 text-xs rounded-xl border border-white/10 bg-black/40 text-zinc-200 focus:outline-none focus:ring-1 focus:ring-amber-500/50 resize-none font-medium placeholder:text-zinc-600"
                                            />
                                        </motion.div>
                                    )}
                                </div>
                                ...
                                <div className="space-y-4 mb-8">
                                    <div className="flex items-center justify-between text-xs">
                                        <span className="text-zinc-500 font-medium tracking-tight">Inventory Subtotal</span>
                                        <span className="font-bold text-zinc-100">{formatPrice(subtotal)}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-xs">
                                        <span className="text-zinc-500 font-medium tracking-tight">Concierge Delivery</span>
                                        {subtotal >= FREE_SHIPPING_THRESHOLD ? (
                                            ...
                                            <span className="text-zinc-200 font-bold">Calculated at checkout</span>
                                        )}
                                    </div>
                                    <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                                        <div>
                                            <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-black block mb-1">Total Valuation</span>
